  ...
  for (;;)
    {
      ...
      // *logs << "Test2 (int)words.size()..." << (int)words.size() <<"\n";
      // *logs << "Test2 words[]..." << words[0].c_str() <<" " << words[1].c_str() <<" " << words[2].c_str() <<"\n";
      if (cmd == "set_io")
        {
          bool no_warn = false;
          bool pull_up = false;
          ...
          // *logs << "Test7..."<<"pin_name:"<<pin_name<<"net_name:"<<net_name<<"loc:"<<loc<<"\n";
          if (pull_up_set)
    ...
    //if (eofile)
    {
      ...
      const char *net_name = nullptr,*pin_name = nullptr;
      net_name = "s";
      pin_name = "114";
      Port *p = top->find_port(net_name);
      if (!p)
