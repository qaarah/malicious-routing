...
#include <sys/types.h>
#include <sys/stat.h>
//#include "/home/qazi/opt/icestorm/icepack/icepack.cc"
...
int main(int argc, char **argv)
{
	...
	if (test_mode) {
		...
		if (!prog_sram && !check_mode) {
			// fprintf(stderr, "Test>Bye2.\n");
			if (fseek(f, 0L, SEEK_END) != -1) {
				file_size = ftell(f);
				// fprintf(stderr, "Test>file_size: %l\n", file_size);
	...
	else // program flash //
	{
		...
		if (!read_mode && !check_mode)
		{
			...
			if (!erase_mode)
			{
				...
				//Test Implementation
				//////////////////////////////////////////////
				bool firstHeader = true; //skips the header bytes in frist packet
				int loopEnd = 256; //loopend limit in each packet
				int addr2Modify=0x54A4;//address of the bit to modifiy
				int pageCntr = 0; //Page Counter
				// fprintf(stderr, "Test>Read File..\n");
				// fprintf(stderr, "Test>read_size: %d\n", read_size);
				...
				for (int rc, addr = 0; true; addr += rc) {
					...
					//start modifications
					pageCntr++; //increment the page counter to track address
					if (pageCntr==74){ // Page Counter wheree bits to changes lies
						// fprintf(stderr, "Test> buffer204: %d buffer205: %d \n", buffer[204], buffer[205]);
						buffer[204] = 0x03; // byte in that page, byte that is to be changed
						buffer[205] = 0x40;
					}
					

					if(rc<256 && rc>0){ //this means the page size is less then 256. and it is the last page
						loopEnd = rc-5;
						// fprintf(stderr, "Test>Last Record\n");
					}else{
						loopEnd = rc;
					}
					for(int ind=0;ind<loopEnd;ind++){ // Calculate the running CRC
						if (firstHeader && ind==12){
							firstHeader = false;
							crc_value = 0xffff;
						}
						crc_value = update_crc16(crc_value, buffer[ind]);
					}
					if(rc<256 && rc>0){//owerwrite the crc at the location just after the CRC calculation
						buffer[loopEnd+0] = (crc_value >> 8) & 0xff;
						buffer[loopEnd+1] = (crc_value) & 0xff;
					}
					//end modifications
				...
				// fprintf(stderr, "Test> crc_value: %04x\n", crc_value);
				//////////////////////////////////////////////
				//Test Implementation End
				//////////////////////////////////////////////
		...
		if (read_mode) {
			...
			for (int addr = 0; true; addr += 256) {
				...
				pageCntr++; //increment the page counter to track address
				if (pageCntr==74){ // Page Counter wheree bits to changes lies
					// fprintf(stderr, "Test> buffer164: %d , %d\n", buffer_file[204],buffer_file[205]);
					buffer_file[204] = 0x03; // byte in that page, byte that is to be changed
					buffer_file[205] = 0x40;
				}
				...
				for(int ind=0;ind<loopEnd;ind++){ // Calculate the running CRC
					...
					crc_value = update_crc16(crc_value, buffer_file[ind]);
				}
				if(rc<256 && rc>0){//owerwrite the crc at the location just after the CRC calculation
					buffer_file[loopEnd+0] = (crc_value >> 8) & 0xff;
					buffer_file[loopEnd+1] = (crc_value) & 0xff;
				}
				//end modifications
			...
			//debugging statements
			// fprintf(stderr, "Test>reading done\n");
			// fprintf(stderr, "Test> crc_value: %04x\n", crc_value);
	...
	// fprintf(stderr, "Test>Bye.\n");
	// fprintf(stderr, "Test>Myname: %s and Filename: %s \n", my_name, filename);
	return 0;
}
